...
{:deps {org.clojure/clojure {:mvn/version "1.12.5"}
        org.duct-framework/main {:mvn/version "0.4.7"}}
...
create either a shell script or shell alias. In a POSIX shell such as
Bash, a shell alias can be created using the `alias` command.
...
The REPL can be left running while source files are updated. The `(reset)`
...
[horizontal]
`--test-config FILE` :: use a custom test config file
`--test-focus ID` :: limit tests to a single test ID, namespace, symbol
                     or metadata keyword
`--test-skip ID` :: skip tests with the test ID, namespace, symbol
                    or metadata keyword

For example, if you wanted to run tests with the `:unit` metadata key,
but exclude the `example.long-tests` namespace:
...
The `--test-focus` and `--test-skip` options may be specified multiple
times.
...
As this is quite a lot to type, you may want to add an extra shell alias.
For example, `ductt` with an extra '`t`' for '`test`'.
...
NOTE: The '`ig`' in `#ig/ref` stands for
...
TIP: At the Duct REPL you can use the `doc` macro to get information
about keywords annotated by Integrant. For example, evaluating the
expression `(doc :duct.module/logging)` will describe the logging module.
...
`:type`    :: a data type to coerce the var into (either `:bool`,
              `:float`, `:int` or `:str`)
...
file. You can place this anywhere in your `duct.edn` configuration.
...
The first thing we'll need is a `deps.edn` file to provide the
...
as a website, using the `:site` feature, and to use the
https://github.com/weavejester/hiccup[Hiccup] format for representing
HTML, using the `:hiccup` feature. We'll also add in a single route to
handle a web request to the root of our application.
...
Then we'll create a Ring **handler** function for that route that
returns a Hiccup data structure.
...
A web module configured with the following routes:
...
popular data-driven routing library for Clojure. Other routing libraries
can be used, but for this documentation we'll use the default.

=== Handlers

Let's take a closer look at the function associated with the route.
...
function will return a full response map, but in this case we're
omitting all but the `:body`, which contains a
...
The `:hiccup` feature of the web module adds middleware to turn Hiccup
vectors into HTML response maps. If the response body is a vector, it
converts it to a string of HTML. The missing `:status` and `:headers`
keys that are usually present in the response map are given default
values.
...
Then we need to incorporate the POST handler and the form into the
codebase.
...
{:vars {jdbc-url {:default "jdbc:sqlite:todo.db"}}
 ...
 {:duct.module/logging {}
  ...
  {:migrations #duct/include "migrations.edn"}
  :duct.module/cljs
  {:main todo.client :output-file "client.js"}
  :duct.module/web
  {:features #{:site :hiccup}
   :handler-opts {:db #ig/ref :duct.database/sql}
   :routes [["/" {:get  :todo.routes/index
...
The module requires two options to be set: `:main`, which references the
main namespace to compile, and `:output-file`, which is the JavaScript
file that the ClojureScript will be compiled to. The `:output-file` is
appended to `:output-path`, which by default is `target/cljs`.

In the above example, this means that the `todo.client` namespace will
be compiled to `target/cljs/client.js`. When Duct is started, this will
be accessible at: <http://localhost:3000/cljs/client.js>.

However, before `todo.client` can be compiled, we first need to write
it. In order to check everything works, we'll just have it trigger a
JavaScript alert.
...
To test this script compiles correctly, we'll add the script to our
`index` function in the `todo.routes` namespace.
...
We then need to change the routes and the module's features. The
`:hiccup` feature is no longer needed as we'll be handling all that from
the client itself, and we'll add the `:api` feature to allow the client
...
There are now three RESTful API routes:
...
around the web server's CSRF protection.
...
A common pattern for using Integrant is to have a `-main` function that loads
...
----
{:aliases
 {:duct {:extra-deps {org.duct-framework/main {:mvn/version "0.4.7"}}
...
of any Integrant configuration. Here are some of the most useful
outside of Duct:
...
Duct makes an effort to integrate nicely with many existing tools,
editors and libraries within the Clojure ecosystem.
...
specify different dependencies for each task. Another advantage is that
a new developer doesn't have to set up a `duct` alias as long as they
have Babashka installed.
...
To do so, you'll need to update your project file with a profile for Duct,
...
(defproject org.example/app "0.1.0-SNAPSHOT"
  ...
  :profiles {:duct {:dependencies [[org.duct-framework/main "0.4.6"]]
                    :main duct.main}})
----

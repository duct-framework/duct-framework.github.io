...
As this is quite a lot to type, you may want to add an exta shell alias.
...
about keywords annotated by Integrant. For example, evaluated the
...
as a webside, using the `:site` feature, and to use the
...
coverts it to a string of HTML. The missing `:status` and `:headers`
...
To test this script compiles correct, we'll add the script to our
...
(defproject org.example/app "0.1.0-SNAPSHOT"
  ...
  :profiles {:duct {:dependencies [[org.duct-framework/main {:mvn/version "0.4.6"]]
                    :main duct.main}})
----
